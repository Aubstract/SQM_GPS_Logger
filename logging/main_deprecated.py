...
# Custom version of csv.DictWriter that ensures the file is flushed to disk after each write
class SafeDictWriter(DictWriter):
    def __init__(self, file_path: Path, *args, **kwargs):
        try:
            ...
            log.critical(f"Data file not found. Please ensure the data directory exists. Data file path: {file_path}")
            raise exc
...
# Variables
DATA_FILE_PATH = Path() # Will be renamed with GPS timestamp once GPS fix is acquired
...
def get_gps_data() -> GPSReport:
    global gps, log, config_file
...
def toggle_trigger_behavior():
    ...
    global trigger_behavior, logging_lock, log
...
def log_measurement():
    global data_file_writer, trigger_id, measurement_id, logging_lock, log
...
def logging_worker():
    global logging_active, extra_measurement, measurements_per_trigger, measurement_interval, trigger_behavior, trigger_id, measurement_id
    while True:
        logging_event.wait()
